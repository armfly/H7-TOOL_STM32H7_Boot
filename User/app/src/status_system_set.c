...
/*
*********************************************************************************************************
*	函 数 名: status_GpioTest
*	功能说明: 测试GPIO输出和TVCC电压调节
*	形    参: 无
*	返 回 值: 无
*********************************************************************************************************
*/
void status_GpioTest(void)
{
	uint8_t ucKeyCode;		/* 按键代码 */
	uint8_t fRefresh;
	FONT_T tFont;		/* 定义字体结构体变量 */
	...
	/* 设置字体参数 */
	{
		...
		LCD_DispStr(5, 3, "GPIO输出测试", &tFont);
	}
	...
	fRefresh = 1;
	while (g_MainStatus == MS_GPIO_TEST)
	{
		bsp_Idle();
		...
		if (fRefresh == 1)	/* 刷新整个界面 */
		{
			...
			bsp_SetTVCC(vcc_volt);
			
			{
				char buf[32];
				
				sprintf(buf, "TVCC = %d.%02dmV", vcc_volt / 1000, (vcc_volt % 1000) / 10);
